 ...
 */
@Component
public class LeaveAction extends GenericActionSupport implements ModelDriven<TestLeaveEntity> {
    private TestLeaveEntity testLeaveEntity;
    private List<TestLeaveEntity> results;
    ...
    @Autowired
    private LeaveWorkflowService leaveWorkflowService; //自定义的服务，用来处理请假流程
    ...
    public String startLeaveProcess(){
        ...
        ProcessInstance processInstance = leaveWorkflowService.startWorkflow(testLeaveEntity, userId, userName, variables);
        return SUCCESS;
    }
    ...
    public String getTaskList(){
        String userId = mSessionMap.get(Constant.USER_ID).toString();
        results = leaveWorkflowService.findTodoTasks(userId);
        return "task_show";
    }
    ...
    public String showTaskView(){
        ...
        testLeaveEntity = leaveWorkflowService.getLeaveEntityById(leaveId);
    ...
    public String finishTask(){
        ...
        leaveWorkflowService.complete(mServletRequest.getParameter("taskId"),variables);
        return SUCCESS;
    }
    ...
    public void initUserData(){
        ...
        Group group = identityService.newGroup("hr");//括号里面是主键，String类型的
        group.setName("人事部门");
        group.setType("assignment");
        identityService.saveGroup(group);

        //创建并保存用户对象 长者
        User user1 = identityService.newUser("5");
        user1.setFirstName("zhang");
        user1.setLastName("zhe");
        identityService.saveUser(user1);

        //创建并保存用户对象 冯淼
        User user2 = identityService.newUser("6");
        user2.setFirstName("ji");
        user2.setLastName("zhe");
        identityService.saveUser(user2);

        //创建组与用户的关联  把他们两个都设为人事
        identityService.createMembership("5","hr");
        identityService.createMembership("6","hr");
    }

    @Override
    public TestLeaveEntity getModel() {
        if (testLeaveEntity==null){
            testLeaveEntity = new TestLeaveEntity();
        }
        return testLeaveEntity;
    }

    public TestLeaveEntity getTestLeaveEntity() {
        return testLeaveEntity;
    }

    public void setTestLeaveEntity(TestLeaveEntity testLeaveEntity) {
        this.testLeaveEntity = testLeaveEntity;
    }

    public List<TestLeaveEntity> getResults() {
        return results;
    }

    public void setResults(List<TestLeaveEntity> results) {
